...
if (!in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
    echo "<div class='error notice'><p>Woocommerce has to be installed and active to use the the Theteller Payment Gateway</b> plugin</p></div>";
    return;
}
...
function woocommerce_theteller_init()
{
    ...
    class WC_Theteller extends WC_Payment_Gateway
    {
        ...
        function init_form_fields()
        {
            $this->form_fields = array(
                ...
                'title' => array(
                    ...
                    'default' => __('Mobile Money Or Card', 'theteller'),
                    'desc_tip' => true
                ),
                'description' => array(
                    'title' => __('Description', 'theteller'),
                    'type' => 'textarea',
                    'desc_tip' => __('Payment description the customer will see during the checkout process.', 'theteller'),
                    'default' => __('Make payments using money from all networks or Ghana issued Visa / Master cards.', 'theteller'),
                    'css' => 'max-width:350px;'
                ),
        ...
        function send_request_to_theteller_api($order_id)
        {
            ...
            if ($status == "success" && $code == "200" && $token != "") {
                ...
                return $checkout_url . "&theteller-response-notice=" . $description;
            }
    ...
    add_filter("plugin_action_links_$plugin", array(
        'WC_Theteller',
        'woocommerce_add_theteller_settings_link'
    ));
    add_filter('woocommerce_payment_gateways', array(
        'WC_Theteller',
        'woocommerce_add_theteller_gateway'
    ));
